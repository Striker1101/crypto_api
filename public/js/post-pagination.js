        ...
        class n extends elementorModules.frontend.handlers.Base {
            ...
            onInitPagination() {
                const t = this.elements.$wrapper,
                    ...
                    l = {
                        action:
                            jkit_element_pagination_option.element_prefix +
                            n.class,
                        data: {
                            current_page: 1,
                            attr: n,
                    ...
                    "yes" === r &&
                        ...
                        i.on("change", function () {
                            const e =
                                "default" !== this.value
                                    ? this.value
                                    : l.data.default_sortby;
                            if (
                                ((l.data.attr.sort_by = e),
                                ["loadmore", "scrollload"].includes(o))
                            )
                                (d.parameter.current_page =
                                    l.data.current_page),
                                    (l.data.current_page = 1),
                                    (l.data.attr.number_post.size +=
                                        l.data.attr.pagination_number_post
                                            .size * c),
                                    m().then(function () {
                                        (l.data.attr.number_post.size =
                                            d.parameter.number_post),
                                            (l.data.current_page =
                                                d.parameter.current_page);
                                    });
                            else if ("disable" === o) {
                                const e = t
                                    .find(
                                        ".jkit-products > .jkit-product-block > .button"
                                    )
                                    .map(function (t) {
